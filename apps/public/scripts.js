...
const API_BASE_URL = 'https://main-server-dot-smart-m-f.de.r.appspot.com/api';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const joinLedgerModalEl = document.getElementById('joinLedgerModal');
    const inviteCodeInputEl = document.getElementById('inviteCodeInput');
    const ledgerPasswordInputEl = document.getElementById('ledgerPasswordInput');
    const confirmJoinLedgerBtnEl = document.getElementById('confirmJoinLedgerBtn');
    const joinLedgerErrorEl = document.getElementById('joinLedgerError');
    const joinLedgerFormEl = document.getElementById('joinLedgerForm'); // 表單元素
    let joinModalInstance; // 用於存儲 Bootstrap Modal 實例
    if (joinLedgerModalEl) {
        joinModalInstance = new bootstrap.Modal(joinLedgerModalEl);

        // 1. 當 Modal 即將顯示時，重置表單和錯誤訊息
        joinLedgerModalEl.addEventListener('show.bs.modal', function() {
            if (joinLedgerFormEl) joinLedgerFormEl.reset(); // 重置表單輸入
            if (joinLedgerErrorEl) {
                joinLedgerErrorEl.textContent = '';
                joinLedgerErrorEl.style.display = 'none';
            }
            if (confirmJoinLedgerBtnEl) {
                confirmJoinLedgerBtnEl.disabled = false;
                confirmJoinLedgerBtnEl.innerHTML = '<i class="fas fa-check me-2"></i>確認加入';
            }
        });

        // 2. 處理「確認加入」按鈕的點擊事件
        if (confirmJoinLedgerBtnEl) {
            confirmJoinLedgerBtnEl.addEventListener('click', async function() {
                const inviteCode = inviteCodeInputEl.value.trim();
                const password = ledgerPasswordInputEl.value; // 密碼可以為空

                // 基本的前端驗證
                if (!inviteCode) {
                    if (joinLedgerErrorEl) {
                        joinLedgerErrorEl.textContent = '請務必輸入邀請碼。';
                        joinLedgerErrorEl.style.display = 'block';
                    }
                    inviteCodeInputEl.focus(); // 將焦點移至邀請碼輸入框
                    return;
                }

                // 設定按鈕為載入中狀態
                confirmJoinLedgerBtnEl.disabled = true;
                confirmJoinLedgerBtnEl.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> 加入中...';
                if (joinLedgerErrorEl) joinLedgerErrorEl.style.display = 'none';
                ...
                try {
                    ...
                    if (result) {
                        ...
                        // 顯示成功訊息 (例如，使用 Toast 通知)
                        // showGlobalSuccessToast(`已成功加入帳本「${result.data?.name || inviteCode}」！`);
                        alert(`已成功加入帳本「${result.data?.name || '共享帳本'}」！`); // 暫用 alert

                        // 重新載入使用者的帳本列表 (假設此函式已存在)
                        if (typeof loadUserLedgers === 'function') {
                            await loadUserLedgers();
                        }

                        // 可選：如果 ledgerSelectModal 仍然開啟，可以考慮關閉它或刷新其內容
                        const ledgerSelectModalEl = document.getElementById('ledgerSelectModal');
                        if (ledgerSelectModalEl) {
                            const ledgerSelectModalInstance = bootstrap.Modal.getInstance(ledgerSelectModalEl);
                            if (ledgerSelectModalInstance && ledgerSelectModalInstance.isShown) {
                                // ledgerSelectModalInstance.hide(); // 例如，直接關閉選擇 Modal
                            }
                        }

                    } else {
                        // API 回報錯誤，但請求本身可能成功 (例如，驗證失敗)
                        throw new Error(result.message || '加入分帳失敗，請檢查邀請碼或密碼。');
                    }
                } catch (error) { // 捕獲 fetch 錯誤或 API 業務邏輯錯誤
                    console.error('加入共享帳本過程中發生錯誤:', error);
                    if (joinLedgerErrorEl) {
                        joinLedgerErrorEl.textContent = error.message;
                        joinLedgerErrorEl.style.display = 'block';
                    }
                    // 發生錯誤時，恢復按鈕狀態
                    confirmJoinLedgerBtnEl.disabled = false;
        ...
        console.warn('ID 為 "joinLedgerModal" 的 Modal 元素未找到。');
    }



    const ledgerSelectModalElement = document.getElementById('ledgerSelectModal');
    if (ledgerSelectModalElement) {
        ledgerSelectModalInstance = new bootstrap.Modal(ledgerSelectModalElement);
    }

    const addLedgerFromModalBtn = document.getElementById('addLedgerFromModalBtn');
    if (addLedgerFromModalBtn) {
        addLedgerFromModalBtn.addEventListener('click', () => {
            if (ledgerSelectModalInstance) {
                ledgerSelectModalInstance.hide(); // 先關閉選擇 Modal
            }
            openAddLedgerModal(); // 再打開新增 Modal
        });
    }

    const confirmDeleteLedgerBtnEl = document.getElementById('confirmDeleteLedgerBtn');

    if (confirmDeleteLedgerBtnEl) {
        confirmDeleteLedgerBtnEl.addEventListener('click', handleDeleteLedgerConfirm);
    }
...
auth.onAuthStateChanged(user => {
    ...
    if (user) {
        ...
        const headerLoggedIn = domCache.headerLoggedIn || document.getElementById('headerLoggedIn');
        const headerLedgerdropdown = domCache.ledgerSwitcherDropdown || document.getElementById('ledgerSwitcherDropdown');
        if (headerLoggedIn) headerLoggedIn.classList.remove('d-none'); // 顯示登入後的 Header
        if (headerLedgerdropdown) headerLedgerdropdown.display = 'block'; // 顯示 Ledger 切換器
...
// --- 輔助函式：用於顯示標準的「請登入」訊息 ---
function showLoginPrompt(element, message = "請先登入以查看此內容。") {
    if (element) {
        element.innerHTML = `<p class="no-account-message text-secondary small p-3 text-center">${message}</p>`;
    }
}

// --- 輔助函式：安全地銷毀 Chart.js 實例 ---
function safeDestroyChart(chartInstance) {
    if (chartInstance && typeof chartInstance.destroy === 'function') {
        chartInstance.destroy();
        return null; // 返回 null 以便重新賦值
    }
    return chartInstance; // 如果不是有效實例，返回原樣
}
...
function resetHeaderForLogout() {
    const headerLoggedIn = domCache.headerLoggedIn || document.getElementById('headerLoggedIn');
    if (headerLoggedIn) headerLoggedIn.style.display = 'none';

    // pageTitle 的更新建議由 handleHashChange 統一處理，因為它依賴於當前 hash
    // const pageTitle = domCache.pageTitle || document.getElementById('pageTitle');
    // if (pageTitle) pageTitle.textContent = "Gochy 錢包"; // 預設應用名稱

    const ledgerSwitcher = domCache.ledgerSwitcherDropdown || document.getElementById('ledgerSwitcherDropdown');
    if (ledgerSwitcher) {
        // ledgerSwitcher.style.display = 'none';
        updateLedgerButtonDisplay('我的主帳本', 'personal'); // 重設按鈕文字
    }
...
function resetHomePageForLogout() {
    const home = domCache.homePage || {}; // 使用 domCache 或直接查詢
    
    // 使用 showLoginPrompt 顯示提示
    const totalAssetsElement = document.querySelector('#totalAssets .value'); // 保持原樣，因結構特殊
    if (totalAssetsElement) totalAssetsElement.textContent = '請登入查看';

    const netAssetsRow = home.netAssetsRow || document.querySelector('.net-assets-row');
    if (netAssetsRow) {
        const liabilityValue = netAssetsRow.querySelector('.liability-value');
        const netAssetValue = netAssetsRow.querySelector('.net-asset-value');
        if (liabilityValue) liabilityValue.textContent = 'NT$ 0';
        if (netAssetValue) netAssetValue.textContent = 'NT$ 0';
    }

    showLoginPrompt(home.dailyExpensesContainer || document.getElementById('dailyExpensesContainer'), "登出狀態，無當日收支");
    showLoginPrompt(home.aiMessages || document.getElementById('aiMessages'), "請登入以獲取 AI 建議");

    // 銷毀圖表
    assetChart = safeDestroyChart(assetChart);
    expenseChart = safeDestroyChart(expenseChart); // 首頁的 expenseChart
    clearCanvas(home.assetChartCanvas || document.getElementById('assetChart'));
    clearCanvas(home.homeExpenseChartCanvas || document.getElementById('expenseChart'));
}

function resetManagePageForLogout() {
    const manage = domCache.managePage || {};

    showLoginPrompt(manage.expenseListContainer || document.getElementById('expenseListContainer'), "請登入查看開銷明細");
    const loadingIndicator = manage.loadingIndicator || document.getElementById('loading');
    if (loadingIndicator) loadingIndicator.style.display = 'none';

    const summaryCard = manage.summaryCardForLedger || document.getElementById('summaryCard');
    if (summaryCard) summaryCard.style.display = 'none';

    const totalExpenseEl = manage.totalExpenseEl || document.getElementById('totalExpense');
    if (totalExpenseEl) totalExpenseEl.textContent = 'NT$ 0';
    const avgDailyEl = manage.averageDailyExpenseEl || document.getElementById('averageDailyExpense');
    if (avgDailyEl) avgDailyEl.textContent = 'NT$ 0';
    const countEl = manage.transactionCountEl || document.getElementById('transactionCount');
    if (countEl) countEl.textContent = '0';

    const catTableBody = manage.categoryTableBody || document.getElementById('categoryTableBody');
    if (catTableBody) catTableBody.innerHTML = '<tr><td colspan="3" class="no-data text-secondary small">請登入查看</td></tr>';

    // 銷毀圖表
    domCache.expenseCategoryChart = safeDestroyChart(domCache.expenseCategoryChart);
    domCache.expenseTrendsChart = safeDestroyChart(domCache.expenseTrendsChart);
    clearCanvas(manage.manageExpenseCatChartCanvas || document.querySelector('#ledgerExpenseTrackerView canvas#expenseCategoryChart'));
    clearCanvas(manage.manageExpenseTrendChartCanvas || document.querySelector('#ledgerExpenseTrackerView canvas#expenseTrendsChart'));

    showLoginPrompt(manage.stocksCardsArea || document.querySelector('#stocksDataView .stock-cards-area'), "請登入查看證券資料");
    showLoginPrompt(manage.assetsDataViewContainer || document.getElementById('assetsDataView'), "請登入查看資產資料");
    
    const summaryText = manage.controlBarSummary || document.getElementById('controlBarSummary');
    if (summaryText) summaryText.textContent = '總金額: -';
    
    const stockListHeader = manage.stockListHeader || document.querySelector('.stock-list-header');
    if (stockListHeader) stockListHeader.style.display = 'none';
}

function resetSharePageForLogout() {
    const share = domCache.sharePage || {};
    showLoginPrompt(share.postsContainer || document.getElementById('postsContainer'), "請登入以瀏覽或發布貼文");
    const loadingMsg = share.loadingPostsMsg || document.getElementById('loadingPosts');
    if (loadingMsg) loadingMsg.style.display = 'none';
}

function resetChargePageForLogout() {
    const charge = domCache.chargePage || {};
    const accForm = charge.accountForm || document.getElementById('accountForm');
    if (accForm) accForm.reset();
    const assForm = charge.assetForm || document.getElementById('assetForm');
    if (assForm) assForm.reset();
    
    initDate(); // 重設日期
    showOrHideMemberDropdown(false); // 隱藏成員下拉
}

function resetNavigationForLogout() {
    // navButtons 可能在 domCache 中，或者需要重新查詢
    const navButtons = domCache.navButtons && domCache.navButtons.length > 0 
                       ? domCache.navButtons 
    ...
    document.title = 'Gochy | SmartFin'; // 預設應用程式標題
}

function resetGlobalStatesForLogout() {
    // 重設帳本狀態
    currentLedger = { id: 'expenses', name: '我的主帳本', type: 'personal' };
    updateLedgerButtonDisplay(currentLedger.name, currentLedger.type); // 更新 Header 按鈕
    ...
    const defaultLedgerOption = menu?.querySelector(`.ledger-dropdown-item[data-ledger-id="${currentLedger.id}"][data-ledger-type="${currentLedger.type}"]`);
    if (defaultLedgerOption) {
        defaultLedgerOption.classList.add('active');
    } else {
        // 如果下拉選單還未填充或找不到預設項，確保按鈕顯示正確
        // (updateLedgerButtonDisplay 已經處理了按鈕本身)
    }
    ...
    hasMore = true;
    isLoading = false;
    isLogin = false; // 標記為未登入
}


// --- 優化後的 updateUIAfterLogout ---
function updateUIAfterLogout() {
    console.log("Optimized: Updating UI to logged-out state and clearing user data...");

    // 1. 切換 CSS class 控制的元素顯示狀態
    ...
    guestDependentElements.forEach(el => {
        // 移除 d-none (如果使用 Bootstrap 控制)，或者恢復預設 display
        el.classList.remove('d-none');
        // 如果不是用 d-none，則需要根據元素類型設定 display
    ...
    userDependentElements.forEach(el => {
        el.classList.add('d-none'); // 使用 Bootstrap 的 d-none class 隱藏
        // el.style.display = 'none';
    });

    // 2. 重設各個主要區塊
    resetHeaderForLogout();
    resetHomePageForLogout();
    resetManagePageForLogout();
    resetSharePageForLogout();
    resetChargePageForLogout();
    
    // 3. 重設導航和全域狀態
    resetNavigationForLogout();
    resetGlobalStatesForLogout();

    console.log("Optimized: UI cleared and reset to logged-out state.");
}
// --- 設定 Modal 切換邏輯 ---
...
function updateUIAfterLogin(user) {
    ...
    if (headerContainer) {
        ...
        // logoutBtn.style.marginLeft = '15px'; // 另一種推到右邊的方式
        ...
        if(currentLedger.type == 'shared'){
            var sharedLedgerInviteCodeContainer = document.getElementById("sharedLedgerInviteCodeContainer");
            if (sharedLedgerInviteCodeContainer) {
                sharedLedgerInviteCodeContainer.style.display = "inline-block"; // 或者 "block"，根據您的佈局需求
            }
            // 1. 透過 ID 選取 <strong> 元素
            var inviteCodeDisplayElement = document.getElementById("inviteCodeDisplay");
            if (inviteCodeDisplayElement) { // 確保元素存在
                inviteCodeDisplayElement.textContent = currentLedger.id;
            } else {
            console.error("找不到 ID 為 'inviteCodeDisplay' 的元素。");
            }
        }else{
            var sharedLedgerInviteCodeContainer = document.getElementById("sharedLedgerInviteCodeContainer");
            if (sharedLedgerInviteCodeContainer) {
                sharedLedgerInviteCodeContainer.style.display = "none"; 
            }
        }
...
/**
 * (新增) 輔助函數：根據成員的墊付金額和平均費用，計算分帳建議。
 * @param {object} membersAfford - 物件，鍵是成員名稱，值是該成員已墊付的金額。
 * @param {Array<string>} memberNames - 帳本中的所有成員名稱列表。
 * @returns {object} 包含總支出、每人平均、每個人的結餘，以及建議的支付列表。
 * 例如: { totalExpense, averageCost, balances: {memberA: 10, memberB: -10}, suggestions: ["MemberB owes MemberA $10"] }
 */
function calculateSplitSuggestions(membersAfford, memberNames) {
    if (!memberNames || memberNames.length === 0) {
        return {
            totalExpense: 0,
            averageCost: 0,
            balances: {},
            suggestions: ["沒有成員資訊，無法計算分帳。"]
        };
    }

    let totalSharedExpenseInPeriod = 0;
    const balances = {}; // 用於儲存每個成員的最終結餘 (已付 - 應付)

    // 確保 membersAfford 包含所有 memberNames，並計算總支出
    memberNames.forEach(name => {
        const amountPaid = membersAfford[name] || 0; // 如果某成員沒付錢，設為0
        totalSharedExpenseInPeriod += amountPaid;
        // 初始化 balances，稍後會減去平均費用
        balances[name] = amountPaid; 
    });

    const numberOfMembers = memberNames.length;
    const averageCostPerMember = numberOfMembers > 0 ? totalSharedExpenseInPeriod / numberOfMembers : 0;

    console.log("Total shared expense in period for split:", totalSharedExpenseInPeriod);
    console.log("Number of members for split:", numberOfMembers);
    console.log("Average cost per member for split:", averageCostPerMember);

    // 計算每個人的最終結餘 (已付 - 應付)
    memberNames.forEach(name => {
        balances[name] = (balances[name] || 0) - averageCostPerMember;
    });

    console.log("Member balances (Paid - Average) for split:", balances);

    // --- 簡化版的分帳建議邏輯 ---
    const suggestions = [];
    // 創建一個可修改的結餘副本，用於處理支付
    let mutableBalances = JSON.parse(JSON.stringify(balances)); 
    
    const debtors = []; // 欠款人 (balance < 0)
    const creditors = []; // 收款人 (balance > 0)

    memberNames.forEach(name => {
        if (mutableBalances[name] < -0.01) { // 允許微小誤差 (例如 -0.001 視為已結清)
            debtors.push({ name, amount: -mutableBalances[name] }); // 欠款金額是正數
        } else if (mutableBalances[name] > 0.01) {
            creditors.push({ name, amount: mutableBalances[name] });
        }
    });

    // 按金額排序，方便處理
    debtors.sort((a, b) => b.amount - a.amount); // 欠最多的先處理
    creditors.sort((a, b) => b.amount - a.amount); // 收款最多的先處理
    ...
    while (debtorIndex < debtors.length && creditorIndex < creditors.length) {
        ...
        if (amountToTransfer > 0.01) { // 只處理有意義的金額轉移
            suggestions.push(`${debtor.name} 應付給 ${creditor.name} NT$ ${formatNumber(amountToTransfer.toFixed(0))}`);
    ...
    // 檢查是否有無法完全結清的情況 (通常是小數點誤差或複雜情況下的簡化邏輯所致)
    // 這裡的檢查可以幫助調試，但在簡化模型下，可能總會有微小差異
    // debtors.forEach(d => {
    //     if (d.amount > 0.01) console.warn(`Split calculation warning: Debtor ${d.name} still has an unassigned debt of ${d.amount.toFixed(2)} after simplified pairing.`);
    // });
    // creditors.forEach(c => {
    //     if (c.amount > 0.01) console.warn(`Split calculation warning: Creditor ${c.name} still has an unfulfilled credit of ${c.amount.toFixed(2)} after simplified pairing.`);
    // });

    if (suggestions.length === 0 && totalSharedExpenseInPeriod > 0.01) { // 增加對總支出的判斷
        suggestions.push("所有成員花費已平均或接近平均，無需互相支付。");
    } else if (suggestions.length === 0 && totalSharedExpenseInPeriod < 0.01) {
        suggestions.push("此期間無共享支出。");
    }

    return {
        totalExpense: totalSharedExpenseInPeriod,
        averageCost: averageCostPerMember,
        balances, // 原始結餘 (相對於平均值)
        suggestions // 支付建議列表
    };
}
...
async function updateExpenseTracker() {
    ...
    try {
        ...
        // API 返回的 records 應該是該期間內的所有收支記錄 (包含收入和支出)
        const allRecordsInPeriod = data.records || []; 
        
        // 我們只關心「支出」用於計算分類佔比和趨勢圖
        const expensesOnlyInPeriod = allRecordsInPeriod.filter(record => record.transactionType === '支出');
        // if(currentLedger.type === 'shared') {
        //     let members_afford = {};
        //     for(let i = 0; i < expenses.length; i++) {
        //         let current_member = expenses[i].member;
        //         members_afford[current_member] = members_afford[current_member] || 0;
        //         members_afford[current_member] += parseInt(expenses[i].amount);
        //     }
        //     console.log("Shared ledger members' afford:", members_afford);
        // }

        if(currentLedger.type === 'shared') {
            console.log("Current ledger is shared. Calculating split suggestions.");
            
            // 步驟 1: 計算此期間有支出的成員的墊付金額
            let periodPaymentsByMember = {}; 
            expensesOnlyInPeriod.forEach(expense => {
                if (expense.member) { // 確保 expense 有 member 屬性
                    periodPaymentsByMember[expense.member] = (periodPaymentsByMember[expense.member] || 0) + parseFloat(expense.amount || 0);
                }
            });
            console.log("Shared ledger members' actual payments in period:", periodPaymentsByMember);

            // 步驟 2: 獲取共享帳本的完整成員列表
            let allLedgerMembers = [];
            try {
                const membersResponse = await fetchWithAuth(`${API_BASE_URL}/split_group/${currentLedger.id}/members`);
                if (membersResponse.ok) {
                    const membersData = await membersResponse.json();
                    allLedgerMembers = membersData.members || []; 
                    if (allLedgerMembers.length === 0) {
                        console.warn(`Warning: Ledger ${currentLedger.id} has no members defined. Split calculation will be based on members found in expenses.`);
                        // 如果帳本沒有定義成員，但有支出記錄，使用支出記錄中的成員作為備選
                        if (Object.keys(periodPaymentsByMember).length > 0) {
                            allLedgerMembers = Object.keys(periodPaymentsByMember);
                        }
                    }
                    console.log("Fetched/derived all members for split calculation:", allLedgerMembers);
                } else {
                    console.warn(`Could not fetch members for shared ledger ${currentLedger.id}. Split calculation may use limited member list.`);
                    // 備選：如果 API 失敗，但期間內有支出，使用支出記錄中的成員
                    if (Object.keys(periodPaymentsByMember).length > 0) {
                        allLedgerMembers = Object.keys(periodPaymentsByMember);
                    }
                }
            } catch (e) {
                console.error("Error fetching members for split:", e);
                // 備選：如果 API 錯誤，但期間內有支出，使用支出記錄中的成員
                if (Object.keys(periodPaymentsByMember).length > 0) {
                    allLedgerMembers = Object.keys(periodPaymentsByMember);
                }
            }

            // 步驟 3: 準備最終的墊付資料 (包含所有帳本成員，無支出則為0)
            // 並確保所有在 periodPaymentsByMember 中但不在 allLedgerMembers 中的成員也被考慮 (雖然這情況較少見)
            let finalAffordances = {};
            const uniqueMembersConsidered = new Set([...allLedgerMembers, ...Object.keys(periodPaymentsByMember)]);

            uniqueMembersConsidered.forEach(member => {
                finalAffordances[member] = periodPaymentsByMember[member] || 0;
            });
            
            // 如果 allLedgerMembers 為空 (表示帳本無成員且期間無支出)，則無法分帳
            if (uniqueMembersConsidered.size > 0) {
                const splitResult = calculateSplitSuggestions(finalAffordances, Array.from(uniqueMembersConsidered));
                console.log("Split calculation result:", splitResult);

                // --- 顯示分帳建議到 UI ---
                const splitSuggestionContainer = document.getElementById('splitSuggestionContainer');
                if (splitSuggestionContainer) {
                    // ... (渲染 HTML 的邏輯，與之前相同) ...
                    
                    let html = `<h5 class="mt-4 mb-3">共享帳本 (期間: ${startDate} - ${endDate})</h5>`;
                    // ... (根據 splitResult.totalExpense, averageCost, suggestions, balances 填充 html)
                    html += `<p class="small text-muted">期間總共享支出: NT$ ${formatNumber(splitResult.totalExpense.toFixed(0))}</p>`;
                    html += `<p class="small text-muted">每人平均應付: NT$ ${formatNumber(splitResult.averageCost.toFixed(0))}</p>`;
                    
                    if (splitResult.suggestions.length > 0) {
                        html += '<ul class="list-group list-group-flush">';
                        splitResult.suggestions.forEach(suggestion => {
                            html += `<li class="list-group-item small">${suggestion}</li>`;
                        });
                        html += '</ul>';
                    } else if (splitResult.totalExpense > 0) {
                        html += '<p class="small">所有成員花費已平均，無需互相支付。</p>';
                    }
                     else {
                        html += '<p class="small">此期間無共享支出。</p>';
                    }
                    html += '<h6 class="mt-3 small">成員結餘 (已付 - 應付):</h6><ul class="list-group list-group-flush small">';
                    // 使用 allLedgerMembers 來確保即使某人結餘為0也顯示
                    Array.from(uniqueMembersConsidered).forEach(memberName => {
                        const balance = splitResult.balances[memberName] || 0; // 如果某成員不在 balances 中 (不應發生)，預設為0
                        const balanceClass = balance > 0.01 ? 'text-success' : balance < -0.01 ? 'text-danger' : 'text-muted';
                        html += `<li class="list-group-item d-flex justify-content-between align-items-center">
                                    ${memberName}: 
                                    <span class="${balanceClass} fw-bold">NT$ ${formatNumber(balance.toFixed(0))}</span>
                                 </li>`;
                    });
                    html += '</ul>';

                    if(currentLedger.type === 'shared') {
                        html += '<div class="chart-container mt-3" style="max-height: 350px;">'
                        html += '<canvas id="memberBalancesChart" style="height: 100%;"></canvas>';
                        html += '</div>';
                    }        
                    splitSuggestionContainer.innerHTML = html;
                    splitSuggestionContainer.style.display = 'block'; // 顯示建議容器

                    // --- 渲染成員結餘圖表 ---
                    // 確保 uniqueMembersConsidered 是一個陣列傳遞給圖表函數
                    renderMemberBalancesChart(splitResult.balances, Array.from(uniqueMembersConsidered)); 
                }
            } else {
                console.log("No members defined for shared ledger and no expenses in period, cannot calculate split.");
                const splitSuggestionContainer = document.getElementById('splitSuggestionContainer');
                if (splitSuggestionContainer) {
                     splitSuggestionContainer.innerHTML = '<p class="small text-center p-2">此共享帳本無成員或此期間無支出，無法計算分帳。</p>';
                }
                // 也要確保如果沒有數據，圖表被清空或顯示無數據
                renderMemberBalancesChart({}, []); // 傳遞空數據以清空/顯示無數據提示
            }
        }
        // --- 更新 UI ---
...
function renderHomePageLedgerSummaries(summaries, year, month) {
    const listContainer = document.getElementById('allLedgersMonthlyExpensesListHome');
    const noDataMessage = document.getElementById('noLedgerReportDataMessageHome');
    const titleElement = document.getElementById('homeLedgerSummaryTitle');
    if(!listContainer){
        console.error("renderHomePageLedgerSummaries: listContainer not found.");
        return;
    }
    if(!noDataMessage){
        console.error("renderHomePageLedgerSummaries: noDataMessage not found.");
        return;
    }
    if(!titleElement){
        console.error("renderHomePageLedgerSummaries: titleElement not found.");
        return;
    }

    titleElement.textContent = `【${month}月】支出明細`;
    listContainer.innerHTML = ''; // 清空舊的資料列內容 (表頭也會被清掉，所以要在下面重新加)
    noDataMessage.style.display = 'none';

    if (summaries && summaries.length > 0) {
        // 1. 動態創建並插入表頭列到 listContainer 的最前面
        const headerRow = document.createElement('div');
        headerRow.className = 'ledger-table-row ledger-table-header'; 
        headerRow.innerHTML = `
            <div class="ledger-table-cell ledger-name-column-header">帳本</div>
            <div class="ledger-table-cell ledger-data-cell-header">現金支出</div>
            <div class="ledger-table-cell ledger-data-cell-header">信用卡支出</div>
            <div class="ledger-table-cell ledger-data-cell-header ledger-total-header">總支出</div>
        `;
        listContainer.appendChild(headerRow); // <<< 將表頭加到 listContainer
        let lotal_liabilities = 0;
        let total_expense = 0;
        let total_cash = 0;
        // 2. 渲染資料列 (邏輯不變，但它們會被加到表頭之後)
        summaries.forEach(summary => {
            const itemElement = document.createElement('div');
            // ... (itemElement 的 className 和 innerHTML 設定不變) ...
            itemElement.className = 'ledger-table-row';
            const ledgerTypeDisplay = summary.ledger_type === 'personal' ? '個人' : '共享';
            const cashFormatted = typeof formatNumber === 'function' ? formatNumber(summary.total_cash || 0) : (summary.total_cash || 0).toFixed(2);
            const creditFormatted = typeof formatNumber === 'function' ? formatNumber(summary.total_liabilities || 0) : (summary.total_liabilities || 0).toFixed(2);
            // const totalExpenseFormatted = typeof formatNumber === 'function' ? formatNumber(summary.total_expense || 0) : (summary.total_expense || 0).toFixed(2);
            const totalExpenseFormatted = formatNumber(summary.total_cash + summary.total_liabilities);
            total_expense += summary.total_cash + summary.total_liabilities || 0;
            total_cash += summary.total_cash || 0;
            lotal_liabilities += summary.total_liabilities || 0;
            // --- 根據帳本類型選擇圖示 ---
            let ledgerIconHtml = '';
            if (summary.ledger_type === 'personal') {
                ledgerIconHtml = '<i class="bi bi-person-circle ledger-type-icon text-secondary"></i>';
            } else if (summary.ledger_type === 'shared') {
                ledgerIconHtml = '<i class="bi bi-people-fill ledger-type-icon text-info"></i>';
            }
            // --- 圖示設定結束 ---

            itemElement.innerHTML = `
                <div class="ledger-table-cell ledger-name-cell">
                    <span class="ledger-name-actual">${summary.ledger_name}</span>
                    ${ledgerIconHtml}
                </div>
                <div class="ledger-table-cell ledger-data-cell ledger-cash-data">
                    NT$ ${cashFormatted}
                </div>
                <div class="ledger-table-cell ledger-data-cell ledger-credit-data">
                    NT$ ${creditFormatted}
                </div>
                <div class="ledger-table-cell ledger-data-cell ledger-total-data">
                    NT$ ${totalExpenseFormatted}
                </div>
            `;
        ...
        footerRow.className = 'ledger-table-row ledger-table-footer'; 
        footerRow.innerHTML = `
            <div class="ledger-table-cell cell-container">
                <div class="ledger-table-cell ledger-data-cell-footer">現金支出:</div>
                <div class="ledger-table-cell ledger-data-cell-footer">NT$ ${total_cash}</div>
            </div>
            <div class="ledger-table-cell cell-container">
                <div class="ledger-table-cell ledger-data-cell-footer">信用卡支出:</div>
                <div class="ledger-table-cell ledger-data-cell-footer">NT$ ${lotal_liabilities}</div>
            </div>
            <div class="ledger-table-cell cell-container">
                <div class="ledger-table-cell ledger-data-cell-footer">總計:</div>
                <div class="ledger-table-cell ledger-data-cell-footer ledger-total-footer">NT$ ${total_expense}</div>
            </div>
        `;
        ...
        noDataMessage.textContent = `${year}年 ${month}月無帳本支出資料。`; // 可以更具體地顯示年月
...
async function updateHomePage() {
    ...
    if (!user) {
        ...
        // 確保首頁的帳本列表也清空
        const ledgerListContainerHome = document.getElementById('allLedgersMonthlyExpensesListHome');
        if(ledgerListContainerHome) ledgerListContainerHome.innerHTML = '';
        const noLedgerDataMsgHome = document.getElementById('noLedgerReportDataMessageHome');
        if(noLedgerDataMsgHome) noLedgerDataMsgHome.style.display = 'none';
        return;
    }
    ...
    try {
        ...
        let totalLiabilities = 0; // << 直接從後端獲取總負債
        ...
        console.log(summaryData.all_ledgers_monthly_amount.length)
        for(let i = 0; i < summaryData.all_ledgers_monthly_amount.length; i++){
            let ledgerLiabilities = summaryData.all_ledgers_monthly_amount[i].total_liabilities || 0; // 獲取每個帳本的負債
            totalLiabilities += ledgerLiabilities; // 累加到總負債
        }
        const netAssets = totalAssets - totalLiabilities; // << 計算淨資產

        const allLedgersExpensesList = summaryData.all_ledgers_monthly_amount || [];
        const currentDate = new Date(); // 用於標題和傳遞給渲染函數
        ...
        assetsContainer.innerHTML = `
            ...
            <div class="net-assets-row">
                <div><span class="label">總負債</span><span class="value liability-value">NT$ ${formatNumber(totalLiabilities)}</span></div>
                <div><span class="label">淨資產</span><span class="value net-asset-value" style="color: ${netAssets >= 0 ? 'var(--success)' : 'var(--danger)'};">NT$ ${formatNumber(netAssets)}</span></div>
            </div>

            <div class="ledger-expenses-summary-container mt-3 user-dependent">
                <h4 class="page-section-title-sm mb-2" id="homeLedgerSummaryTitle">各帳本【${currentDate.getMonth() + 1}月】支出</h4>
                <div id="allLedgersMonthlyExpensesLoadingHome" class="text-center my-2" style="display: none;">
                     <div class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">載入中...</span></div>
                     <span class="ms-2 text-secondary small">載入中...</span>
                </div>
                <div id="allLedgersMonthlyExpensesListHome" class="list-group list-group-flush"></div>
                <p id="noLedgerReportDataMessageHome" class="text-center text-secondary small mt-2" style="display: none;"></p>
            </div>

            <div id="dailyExpensesContainer" class="record-card-container user-dependent" style="margin-top: 6px;"></div>
            
            <div id="aiAssistantBoard" class="ai-assistant-container user-dependent">
                <h5 class="ai-assistant-title">Gochy AI</h5>
                <p class="help-text">您的財務小幫手</p>
                <div id="aiMessages" class="ai-messages"><p class="message-placeholder">AI 載入中...</p></div>
            </div>

            <div class="sub-container user-dependent" id="expenseChartDisplayArea" style="margin: 20px auto;">
                <h5 class="text-center chart-title-above-layout">當月開銷分佈</h5> <div class="chart-table-layout">
                    <div class="chart-pie-container">
                        <canvas id="expenseChart"></canvas>
                    </div>
                    <div class="chart-table-container">
                        <table class="table table-sm table-hover distribution-table">
                            <thead>
                                <tr>
                                    <th>類別</th>
                            ...
                            <tbody id="expenseChartTableBody">
                                </tbody>
                        </table>
                    </div>
                </div>
            ...
            <div class="sub-container user-dependent" id="assetChartDisplayArea" style="margin: 10px auto;">
                <h5 class="text-center chart-title-above-layout">總資產分佈</h5> <div class="chart-table-layout">
                    <div class="chart-pie-container">
        ...
        // HTML 重建完成後，才能安全地獲取 ledgerListLoadingIndicator
        const ledgerListLoadingIndicatorAfterRender = document.getElementById('allLedgersMonthlyExpensesLoadingHome');
        if (ledgerListLoadingIndicatorAfterRender) ledgerListLoadingIndicatorAfterRender.style.display = 'none';
        renderHomePageLedgerSummaries(allLedgersExpensesList, currentDate.getFullYear(), currentDate.getMonth() + 1);
        ...
        // 傳入 canvas ID, table body ID, 數據, Chart.js 內部標籤, 全域實例引用名
        renderDistributionOnChart(
            'assetChart', 
            'assetChartTableBody', // 新增：資產表格 body ID
            assetDistribution, 
            '資產分佈', // Chart.js dataset label
            'assetChart'
        );
        renderDistributionOnChart(
            'expenseChart', 
            'expenseChartTableBody', // 新增：開銷表格 body ID
            expenseDistribution, 
            '開銷分佈', // Chart.js dataset label
            'expenseChart'
        );
...
function renderDailyExpenses(expenses, todayTotalExpense) {
    ...
    // 1. 設定標題並清空舊內容 (除了標題)
    container.innerHTML = '<h4 style="text-align: flex-start; font-size:1rem; padding-left: 1rem;">當天收支</h4>'; // 修改標題以反映可能包含收入

    // 2. 檢查是否有當天記錄
    if (!expenses || expenses.length === 0) {
        container.innerHTML += '<p style="text-align: flex-start; font-size:0.7rem; padding-left: 1rem; color: var(--text-secondary);">今日無收支記錄</p>';
    ...
    totalCard.innerHTML = `
        <div class="record-Item-title" style="font-weight: bold; padding-left:1rem;"> <span>收支紀錄</span>
...
/**
 * 將分佈數據渲染到指定的 Canvas 上 (圓餅圖) 並填充旁邊的表格。
 * @param {string} canvasId - 要繪製圖表的 <canvas> 元素的 ID。
 * @param {string} tableBodyId - 要填充數據的 <tbody> 元素的 ID。
 * @param {object} distributionData - 包含分佈數據的物件，鍵是標籤，值是數值。
 * @param {string} chartJsTitle - Chart.js 內部使用的標題 (現在可能被手動標題取代)。
 * @param {string} chartInstanceRefName - 用於存儲此圖表實例的全域變數名稱 (例如 'assetChart')。
 */
function renderDistributionOnChart(canvasId, tableBodyId, distributionData, chartJsTitle, chartInstanceRefName) {
    const displayAreaId = canvasId.replace('Chart', 'ChartDisplayArea'); // 例如 expenseChart -> expenseChartDisplayArea
    const displayArea = document.getElementById(displayAreaId);

    // 檢查最外層容器是否存在
    if (!displayArea) {
        console.error(`renderDistributionOnChart: Display area #${displayAreaId} not found.`);
        return;
    }
    ...
    if (!canvasElement) {
        console.error(`renderDistributionOnChart: Canvas element #${canvasId} not found.`);
        if (tableBody) tableBody.innerHTML = '<tr><td colspan="3" class="text-center text-danger small">圖表畫布缺失</td></tr>';
        return;
    }
    if (!tableBody) {
        console.error(`renderDistributionOnChart: Table body #${tableBodyId} not found.`);
        // 即使表格缺失，圖表仍可嘗試渲染
    }
    ...
    if (!ctx) {
        console.error(`renderDistributionOnChart: Failed to get 2D context for canvas "${canvasId}".`);
        if (tableBody) tableBody.innerHTML = '<tr><td colspan="3" class="text-center text-danger small">無法渲染圖表</td></tr>';
        return;
    }

    // --- 銷毀舊圖表實例 ---
    let oldChartInstance = null;
    if (chartInstanceRefName === 'assetChart') oldChartInstance = assetChart;
    else if (chartInstanceRefName === 'expenseChart') oldChartInstance = expenseChart;

    if (oldChartInstance instanceof Chart) {
        oldChartInstance.destroy();
        if (chartInstanceRefName === 'assetChart') assetChart = null;
        else if (chartInstanceRefName === 'expenseChart') expenseChart = null;
    }
    ...
    const values = [];
    let totalAmount = 0;
    const validEntries = []; // 用於排序和表格

    if (distributionData && typeof distributionData === 'object') {
        for (const key in distributionData) {
            if (distributionData.hasOwnProperty(key) && parseFloat(distributionData[key]) > 0) {
                const value = parseFloat(distributionData[key]);
                validEntries.push({ label: key, value: value });
                totalAmount += value;
            }
        }
    }
    ...
    // --- 檢查是否有數據可供顯示 ---
    if (labels.length === 0) {
        console.log(`No data available to render chart and table for: ${chartJsTitle}`);
        clearCanvas(canvasElement); // 清空畫布
        ctx.save();
        ctx.fillStyle = 'grey'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = '14px Arial';
        ctx.fillText('無數據', canvasElement.width / 2, canvasElement.height / 2);
        ctx.restore();
        if (tableBody) tableBody.innerHTML = '<tr><td colspan="3" class="text-center text-muted small p-3">此期間無相關數據</td></tr>';
        return;
    }
    ...
    if (tableBody) {
        ...
        validEntries.forEach(entry => {
            ...
            row.innerHTML = `
                <td>${entry.label}</td>
                <td class="text-end amount-column">${formatNumber(entry.value.toFixed(0))}</td>
                <td class="text-end percentage-column">${percentage}%</td>
            `;
    ...
    const newChart = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: chartJsTitle, // Tooltip 中顯示的數據集標籤
                data: values,
                backgroundColor: backgroundColors,
                borderColor: '#ffffff',
                borderWidth: 1
        ...
        options: {
            responsive: true,
            maintainAspectRatio: false, // 關鍵，允許 canvas 調整大小以適應父容器
            plugins: {
                legend: { // 圓餅圖的圖例通常在右側或底部效果較好，或者直接隱藏，依賴表格
                    display: true, // 或 false，如果表格已足夠清晰
                    position: 'right', 
                    labels: { boxWidth: 12, padding: 10, font: {size: 10} }
                },
                title: {
                    display: false, // 我們在 HTML 中手動添加了標題
                    // text: chartJsTitle, // Chart.js 標題 (如果還需要的話)
                },
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            return `${label}: NT$ ${formatNumber(value.toFixed(0))} (${percentage}%)`;
                        }
                    }
                }
            },
            // 可以考慮為甜甜圈圖設定 cutoutPercentage
            // cutout: '60%', 
        }
    });

    if (chartInstanceRefName === 'assetChart') assetChart = newChart;
    else if (chartInstanceRefName === 'expenseChart') expenseChart = newChart;
    
    console.log(`Chart '${chartJsTitle}' and table '${tableBodyId}' rendered successfully.`);
}
...
// --- 全域變數 (用於刪除 Modal) ---
let ledgerToDeleteInfo = { id: null, name: null, type: null };
let deleteLedgerModalInstance = null; // 儲存刪除 Modal 實例
let ledgerSelectModalInstance = null; // 全域變數
...
//                 <i class="bi bi-trash delete-ledger-icon"></i>
//             </span>
//             `;
...
/**
 * 填充「選擇帳本」模態框的內容。
 * @param {object} ledgersData - 從後端獲取的帳本數據，包含 personal 和 shared 兩個陣列。
 * 例如: { personal: ["主帳本", "旅遊基金"], shared: [{invite_code: "ABC12", name: "家庭共享"}] }
 */
function populateLedgerSwitcher(ledgersData) {
    const personalLedgers = ledgersData && Array.isArray(ledgersData.personal) ? ledgersData.personal : [];
    const sharedLedgers = ledgersData && Array.isArray(ledgersData.shared) ? ledgersData.shared : [];
    const modalBody = document.getElementById('ledgerListContainerInModal');

    if (!modalBody) {
        console.error("錯誤：找不到 Modal 內的帳本列表容器 (#ledgerListContainerInModal)。");
        return;
    }

    // 清空上一次的列表內容 (保留 "讀取中..." 或錯誤訊息，如果有的話，將在下方處理)
    modalBody.innerHTML = '';

    const fragment = document.createDocumentFragment(); // 用於高效能地附加元素

    /**
     * 輔助函數：創建單個帳本列表項的 DOM 元素。
     * @param {string} id - 帳本的唯一標識符。
     * @param {string} name - 帳本的顯示名稱。
     * @param {'personal' | 'shared'} type - 帳本類型。
     * @returns {HTMLElement} 創建的帳本列表項元素。
     */
    const createLedgerItemElement = (id, name, type) => {
        const itemDiv = document.createElement('div');
        itemDiv.className = 'ledger-modal-item'; // 自訂 class 用於樣式
        itemDiv.dataset.ledgerId = id;
        itemDiv.dataset.ledgerName = name;
        itemDiv.dataset.ledgerType = type;
        itemDiv.setAttribute('role', 'button'); // 提高可訪問性
        itemDiv.tabIndex = 0; // 允許鍵盤聚焦

        // 帳本圖示和名稱
        const itemContent = document.createElement('span');
        itemContent.className = 'item-content';
        itemContent.innerHTML = `
            <span class="item-icon">${getLedgerIconHTML(type)}</span>
            <span class="item-name">${name}</span>
        `;

        // 刪除圖示
        const deleteIcon = document.createElement('i');
        deleteIcon.className = 'bi bi-trash delete-ledger-icon';
        deleteIcon.title = `刪除帳本 ${name}`;
        ...
        // 檢查是否為當前選中的帳本，並添加相應的 class
        if (currentLedger && currentLedger.id === id && currentLedger.type === type) {
            itemDiv.classList.add('active');
        }

        // 為整個列表項綁定「選擇帳本」的事件監聽器
        itemDiv.addEventListener('click', function(event) {
            // 確保點擊的不是刪除圖示本身
            if (event.target === deleteIcon || deleteIcon.contains(event.target)) {
                return;
            }

            // 更新全域的 currentLedger 狀態
            currentLedger = {
                id: this.dataset.ledgerId,
                name: this.dataset.ledgerName,
                type: this.dataset.ledgerType
            };

            // 更新 Header 中按鈕的顯示
            updateLedgerButtonDisplay(currentLedger.name, currentLedger.type);

            // 更新 Modal 內部列表項的 'active' 狀態
            const allModalItems = modalBody.querySelectorAll('.ledger-modal-item');
            allModalItems.forEach(activeItem => activeItem.classList.remove('active'));
            this.classList.add('active');

            // 關閉「選擇帳本」的 Modal
            if (ledgerSelectModalInstance) {
                ledgerSelectModalInstance.hide();
            }
            ...
            // console.log(`帳本已切換至 (從 Modal): ${currentLedger.name} (${currentLedger.type})`);
        });

        // 為刪除圖示綁定獨立的事件監聽器
        deleteIcon.addEventListener('click', (event) => {
            event.stopPropagation(); // 非常重要：阻止事件冒泡到父元素(itemDiv)的點擊事件
            // console.log(`嘗試刪除帳本: ID=${id}, Name=${name}, Type=${type}`);
            openDeleteLedgerModal(id, name, type); // 調用已有的函數打開刪除確認 Modal
        });
        // 允許鍵盤觸發刪除圖示
        deleteIcon.addEventListener('keydown', (event) => {
            if (event.key === 'Enter' || event.key === ' ') {
                ...
                openDeleteLedgerModal(id, name, type);
            }
        });
    ...
    if (personalLedgers.length > 0) {
        ...
        personalHeader.className = 'text-muted small px-3 pt-3 pb-1 ledger-group-header'; // 添加 header class
        personalHeader.textContent = '個人帳本';
        fragment.appendChild(personalHeader);

        personalLedgers.forEach(ledgerName => { // 對於個人帳本，名稱即ID
            fragment.appendChild(createLedgerItemElement(ledgerName, ledgerName, 'personal'));
        });
    }
    ...
    if (sharedLedgers.length > 0) {
        ...
        sharedHeader.className = 'text-muted small px-3 pt-3 pb-1 ledger-group-header'; // 添加 header class
        sharedHeader.textContent = '共享帳本';
        fragment.appendChild(sharedHeader);

        sharedLedgers.forEach(ledgerInfo => {
            if (ledgerInfo && ledgerInfo.invite_code && ledgerInfo.name) {
                fragment.appendChild(createLedgerItemElement(ledgerInfo.invite_code, ledgerInfo.name, 'shared'));
            } else {
                console.warn("無效的共享帳本數據:", ledgerInfo);
            }
        });
    }

    // 如果沒有任何帳本，顯示提示訊息
    if (personalLedgers.length === 0 && sharedLedgers.length === 0) {
        const noLedgersMessage = document.createElement('p');
        noLedgersMessage.className = 'text-center text-muted p-3';
        noLedgersMessage.textContent = '尚無可用帳本。您可以點擊下方的「新增帳本」按鈕來建立一個新的帳本。';
        fragment.appendChild(noLedgersMessage);
    }

    modalBody.appendChild(fragment); // 一次性將所有元素添加到 Modal Body 中
    // console.log("帳本選擇 Modal 已成功填充列表。");
}
 ...
 * @param {'personal' | 'shared'} ledgerType
 */
function openDeleteLedgerModal(ledgerId, ledgerName, ledgerType) {

    const ledgerSelectModal = document.getElementById('ledgerSelectModal');
    ledgerSelectModalInstance = bootstrap.Modal.getInstance(ledgerSelectModal);
    if(ledgerSelectModalInstance) {
        ledgerSelectModalInstance.hide(); // 隱藏帳本選擇 Modal
        // ledgerSelectModal.classList.remove('show'); // 這行可能會導致顯示問題
    }
    // ledgerSelectModal.style.display = 'none';

    console.log(`Opening delete confirmation for ledger: ${ledgerName} (ID: ${ledgerId}, Type: ${ledgerType})`);
    
    ledgerToDeleteInfo = { id: ledgerId, name: ledgerName, type: ledgerType }; // 儲存待刪除資訊

    const modalElement = document.getElementById('deleteLedgerModal');
    if (!modalElement) { console.error("#deleteLedgerModal not found!"); return; }

    // 填充 Modal 內容
    document.getElementById('ledgerNameToDelete').textContent = ledgerName;
    document.getElementById('ledgerNameToConfirm').textContent = ledgerName; // 用於確認輸入提示
    const confirmInput = document.getElementById('confirmLedgerNameInput');
    const confirmButton = document.getElementById('confirmDeleteLedgerBtn');
    const errorDiv = document.getElementById('deleteLedgerError');

    if (confirmInput) confirmInput.value = ''; // 清空輸入框
    if (errorDiv) errorDiv.style.display = 'none'; // 隱藏錯誤
    if (confirmButton) confirmButton.disabled = true; // 預設禁用確認按鈕

    // 監聽輸入框，比對名稱以啟用按鈕
    if (confirmInput && confirmButton) {
         // 移除舊監聽器，避免重複
         confirmInput.oninput = null; // 簡單移除
         confirmInput.oninput = () => {
             if (confirmInput.value === ledgerName) {
    ...
    deleteLedgerModalInstance = bootstrap.Modal.getOrCreateInstance(modalElement);
    deleteLedgerModalInstance.show();
}


async function handleDeleteLedgerConfirm() {
    const user = auth.currentUser;
    if (!user) { alert("請先登入"); return; }

    const { id, name, type } = ledgerToDeleteInfo;
    if (!id || !name || !type) {
        alert("刪除錯誤：缺少帳本資訊。");
        return;
    }
    ...
    if (confirmInput.value !== name) {
        if(errorDiv) {
            errorDiv.textContent = "輸入的帳本名稱不匹配！";
    ...
    if (confirmButton) {
        confirmButton.disabled = true;
        confirmButton.innerHTML = '<span class="spinner-border spinner-border-sm"></span> 刪除中...';
    }

    try {
        // 後端 API 端點：DELETE /api/ledger/{ledger_id}?type={ledger_type}
        const response = await fetchWithAuth(`${API_BASE_URL}/ledger/delete?ledgerId=${encodeURIComponent(id)}&ledgerName=${encodeURIComponent(name)}&ledgerType=${encodeURIComponent(type)}`, {
            method: 'DELETE'
        });

        const result = await response.json();
        if (!response.ok) {
            throw new Error(result.error || `刪除失敗 (${response.status})`);
        }

        // --- 刪除成功 ---
        alert(`帳本「${name}」已成功刪除！`);
        if (deleteLedgerModalInstance) deleteLedgerModalInstance.hide();

        // 刷新帳本下拉選單
        await loadUserLedgers();

        // 檢查是否刪除了當前選中的帳本
        if (currentLedger.id === id && currentLedger.type === type) {
            console.log("Current ledger was deleted. Switching to default.");
            // 切換到預設帳本 (例如第一個個人帳本，或定義的 'default_personal')
            // 這部分邏輯需要根據 loadUserLedgers 後的結果來設定
            // 暫時先設為一個預設 ID，loadUserLedgers 成功後會更新按鈕顯示
            currentLedger = { id: 'default_personal', name: '我的主帳本', type: 'personal' }; // 需要確保這個預設帳本存在
            updateLedgerButtonDisplay(currentLedger.name, currentLedger.type);
            handleHashChange(user); // 刷新當前頁面數據
        }
        ...
        if (errorDiv) {
            errorDiv.textContent = `刪除失敗：${error.message}`;
            errorDiv.style.display = 'block';
        } else {
...
let memberBalancesChartInstance = null; // 全域變數儲存圖表實例

/**
 * 渲染成員結餘的水平長條圖。
 * @param {object} balances - 物件，鍵是成員名稱，值是該成員的結餘金額。
 * 正數表示應收，負數表示應付。
 * @param {Array<string>} memberNames - 帳本中的所有成員名稱列表，用於確定順序和標籤。
 */
function renderMemberBalancesChart(balances, memberNames) {
    const canvasId = 'memberBalancesChart';
    const canvasElement = document.getElementById('memberBalancesChart');
    if (!canvasElement) {
        console.error(`renderMemberBalancesChart: Canvas element #${canvasId} not found.`);
        return;
    }

    const ctx = canvasElement.getContext('2d');
    if (!ctx) {
        console.error(`renderMemberBalancesChart: Failed to get 2D context for #${canvasId}.`);
        return;
    }

    // 銷毀舊的圖表實例
    if (memberBalancesChartInstance && typeof memberBalancesChartInstance.destroy === 'function') {
        memberBalancesChartInstance.destroy();
        memberBalancesChartInstance = null;
    }

    if (!balances || Object.keys(balances).length === 0 || !memberNames || memberNames.length === 0) {
        console.log("No balance data or member names to render member balances chart.");
        // 清空畫布並顯示提示
        clearCanvas(canvasElement); // 假設 clearCanvas 函數已定義
        ctx.save();
        ctx.fillStyle = 'grey';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.font = '14px Arial';
        ctx.fillText('無成員結餘數據', canvasElement.width / 2, canvasElement.height / 2);
        ctx.restore();
        return;
    }
    ...
    // 按照 memberNames 的順序來確保圖表標籤順序一致
    memberNames.forEach(name => {
        const balance = balances[name] || 0; // 如果某成員不在 balances 中，視為0
        labels.push(name);
        dataValues.push(balance); // 直接使用原始結餘（可正可負）

        if (balance > 0.01) { // 應收款 (多付)
            backgroundColors.push('rgba(75, 192, 192, 0.6)'); // 例如：綠色系
            borderColors.push('rgba(75, 192, 192, 1)');
        } else if (balance < -0.01) { // 應付款 (少付)
            backgroundColors.push('rgba(255, 99, 132, 0.6)'); // 例如：紅色系
            borderColors.push('rgba(255, 99, 132, 1)');
        } else { // 結餘接近0
            backgroundColors.push('rgba(201, 203, 207, 0.6)'); // 例如：灰色系
            borderColors.push('rgba(201, 203, 207, 1)');
        }
    });
    
    // 動態調整 Canvas 的父容器高度，基於成員數量
    // 這樣可以避免成員過多時圖表顯得擁擠
    const chartContainer = canvasElement.parentElement;
    if (chartContainer) {
        const minBarHeight = 30; // 每個長條的最小高度 (px)
        const estimatedHeight = memberNames.length * minBarHeight + 60; // +60 給上下邊距和標題
        // chartContainer.style.height = `${Math.min(Math.max(estimatedHeight, 150), 400)}px`; // 限制最小150px, 最大400px
        // 或者，如果canvas本身是響應式的，就不需要手動設定容器高度，
        // Chart.js 會嘗試填滿容器。但對於水平長條圖，控制高度有時是必要的。
        // 如果使用 maintainAspectRatio: false，則 canvas 會填滿其父容器
    }


    memberBalancesChartInstance = new Chart(ctx, {
        type: 'bar', // 水平長條圖，在 options 中設定 indexAxis: 'y'
        data: {
            labels: labels,
            datasets: [{
                label: '成員結餘 (NT$)',
        ...
        options: {
            indexAxis: 'y', // <--- 設置為水平長條圖的關鍵
            responsive: true,
            maintainAspectRatio: false, // 允許圖表不按比例縮放以填滿容器
            scales: {
                x: {
                    ...
                    ticks: {
                        callback: function(value, index, values) {
                            return formatNumber(value); // 格式化 X 軸刻度
                        }
                    }
                },
                y: {
                    // 如果成員名稱太長，可以考慮調整 ticks
                    // ticks: { autoSkip: false } // 顯示所有標籤
                }
            },
            plugins: {
                legend: {
                    display: false // 通常對於單一數據集的長條圖，圖例不是很有必要
                },
                title: {
                    display: true,
                    text: '成員結餘狀況 (正數表示應收，負數表示應付)',
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            if (value > 0) {
                                label += `應收 NT$ ${formatNumber(value.toFixed(0))}`;
                            } else if (value < 0) {
                                label += `應付 NT$ ${formatNumber(Math.abs(value).toFixed(0))}`;
                            } else {
                                label += `結清 NT$ ${formatNumber(value.toFixed(0))}`;
                            }
                            return label;
                        }
                    }
